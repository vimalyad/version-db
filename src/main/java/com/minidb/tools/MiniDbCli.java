...
/**
 * A line-oriented REPL over {@link MiniDb}. Each input line is one command:
...
public final class MiniDbCli {
    ...
    private static String format(ResultSet rs) {
        ...
        for (List<Value> r : rs.rows()) {
            sb.append('\n').append(row(r.stream().map(MiniDbCli::cell).toList()));
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (MiniDb db = MiniDb.open(dir);
             BufferedReader in = new BufferedReader(new InputStreamReader(System.in, StandardCharsets.UTF_8))) {
            MiniDbCli cli = new MiniDbCli(db);
            System.out.println("MiniDB ready. End statements with ';'. Type 'exit' to quit.");
